...
public class Utilitaries {
	...
	public String compareSimpleTime(long current, long end) {
		...
		String retur = (days > 0 ? days + " d, " : "") + (hours > 0 ? hours + " h, " : "") + (minutes > 0 ? minutes + " m, " : "") + (seconds > 0 ? seconds + " s," : "");
	...
	public String captalize(String toCaptalize) {
		return toCaptalize.substring(0, 1).toUpperCase() + toCaptalize.substring(1).toLowerCase();
	}
	...
	public long parseDateDiff(String time, boolean future) throws Exception {
		Pattern timePattern = Pattern.compile(
				"(?:([0-9]+)\\s*y[a-z]*[,\\s]*)?(?:([0-9]+)\\s*mo[a-z]*[,\\s]*)?(?:([0-9]+)\\s*w[a-z]*[,\\s]*)?(?:([0-9]+)\\s*d[a-z]*[,\\s]*)?(?:([0-9]+)\\s*h[a-z]*[,\\s]*)?(?:([0-9]+)\\s*m[a-z]*[,\\s]*)?(?:([0-9]+)\\s*(?:s[a-z]*)?)?", 2);
